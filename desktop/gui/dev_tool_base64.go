...
func (d *devToolBase64) CreateView() fyne.CanvasObject {
	...
	d.encodeBtn = widget.NewButtonWithIcon(sidTheme.AppDevToolsBase64EncodeName, sidTheme.ResourceEncodeIcon, func() {
		enc := d.getEncoding()
		out := enc.EncodeToString([]byte(d.inputText.Text))
		d.outputText.SetText(out)
	})
	d.decodeBtn = widget.NewButtonWithIcon(sidTheme.AppDevToolsBase64DecodeName, sidTheme.ResourceDecodeIcon, func() {
		enc := d.getEncoding()
	...
	d.urlChk = widget.NewCheck(sidTheme.AppDevToolsBase64UrlModeName, nil)
	d.urlChk.SetChecked(false)

	d.fillChk = widget.NewCheck(sidTheme.AppDevToolsBase64FillModeName, nil)
